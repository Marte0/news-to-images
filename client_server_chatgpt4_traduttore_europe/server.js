...
const image_generate = async (instruction, description, retry = 0) => {
  try {
    ...
    console.log(id_image + " , " + imageUrl + " , " + bucketName + " , " + pathInBucket + " , " + description);
...
async function insertImageRecord(id_image, imagePath, description, state, timeZone, date_time) {
  const { data, error } = await supabase.from("europe").insert([{ id: id_image, href: imagePath, description: description, state: state, time_zone: timeZone, date_time: date_time }]);
...
// let url = "https://oaidalleapiprodscus.blob.core.windows.net/private/org-awOJkPr1OOqOOPrAo2CSyDC2/user-HR83uB4E76U41I5Kqtk2FFWB/img-bd4qbnvpuMdzeOuqfm7waiYb.png?st=2024-03-08T09%3A02%3A57Z&se=2024-03-08T11%3A02%3A57Z&sp=r&sv=2021-08-06&sr=b&rscd=inline&rsct=image/png&skoid=6aaadede-4fb3-4698-a8f6-684d7786b067&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skt=2024-03-07T18%3A07%3A25Z&ske=2024-03-08T18%3A07%3A25Z&sks=b&skv=2021-08-06&sig=%2Bw6lx0OdQkohdWjH2i1N0YAYdeYfMcC73ePCdgObwCo%3D"

// uploadImageToSupabase(1709807065050, url, "news-to-image", "1709807065050.webp", "Piers Morgan sexism row after controversial International Women's Days jibes")
