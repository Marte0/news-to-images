...
async function insertImageRecord(id_image, imagePath, description, state, timeZone, date_time) {
  const { data, error } = await supabase.from("america").insert([{ id: id_image, href: imagePath, description: description, state: state, time_zone: timeZone, date_time: date_time }]);
