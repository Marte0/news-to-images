...
image1.src = "YOUR SUPABASE URL/storage/v1/object/public/news-to-image/1709805069163.jpg";
...
async function updateState(obj) {
  ...
  const { data, error } = await supabase.from("america").upsert({ id: obj.id, used: "TRUE" }).select();
}

async function fetchLastUploadedImage() {
  const { data, error } = await supabase
    .from("america") // Replace with your table name
...
function editCartillo(obj) {
  ...
  document.getElementById("continent").innerHTML = "america";
